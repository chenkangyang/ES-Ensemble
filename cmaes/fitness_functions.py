...
from sklearn.metrics import f1_score # for "ff.water_ensemble"
from sklearn.metrics import accuracy_score # for "ff.water_ensemble"
...
class FitnessFunctions(object):  # TODO: this class is not necessary anymore? But some effort is needed to change it
    ...
    # def water_ensemble(self, x, prob, y_valid):
    def water_ensemble(self, x, prob, y_valid, metric):
        """args"""
        # prob: [base_model_num, n_samples, class_num]
        # y_valid: [n_samples, 1]
        # x(list): [class_num]

        """objective function"""
        base_model_num = prob.shape[0]
        n_samples = y_valid.shape[0]
        class_num = prob.shape[2]
        # map x
        # x = np.exp(-x**2)
        x = np.exp(x)/np.sum(np.exp(x))

        y_ensemble_prob = np.zeros((n_samples, class_num))
        for k in range(base_model_num):
            y_ensemble_prob += prob[k] * x[k]
        y_ensemble_pred = np.argmax(y_ensemble_prob, axis=1)
        f1 = f1_score(y_valid, y_ensemble_pred, average="weighted")
        acc = accuracy_score(y_valid, y_ensemble_pred)
